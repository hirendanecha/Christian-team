...
export function app(): express.Express {
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'dist/christian-team/browser'), 'index.html')
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            name: post?.title || post?.albumname || 'Christian.Team Post',
          ...
          const talent = {
            name: `Christian.Team Research ${group?.PageTitle}`,
